//Вариант 40 Координаты без плоскости
#include<iostream>
#include<math.h>
#include<locale.h>
#include<limits.h>
...
std:: system ("chcp 65001");
 ...
 else if((x+1)*(x+1)+(y-1)*(y-1)<1&&(x+1)*(x+1)+y*y<1&&x*x+y*y>1)
      {cout << "Точка в области М2. " ; i = 2 ;
      }
      else if((x-1)*(x-1)+(y-1)*(y-1)<1&&(x-1)*(x-1)+y*y<1&&x*x+(y-1)*(y-1)>1)
	      {cout << "Точка в области М3. " ; i = 3 ;
	      }
	   else if((x+1)*(x+1)+(y+1)*(y+1)>1&&(x-1)*(x-1)+y*y>1&&x*x+(y+1)*(y+1)<1&&x*x+y*y<1)
